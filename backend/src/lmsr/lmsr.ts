...
export function findQuantityForExactCost(qYes: number, qNo: number, cash: number, b = 10) {
  // Find delta such that costToBuyDeltaYes(qYes,qNo,delta) = cash
  ...
  for (let i = 0; i < maxIter; i++) {
    ...
    const c = costToBuyDeltaYes(qYes, qNo, mid, b);
    if (c > cash) high = mid;
    else low = mid;
  }
  return low;
