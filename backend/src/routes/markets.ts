...
import { costToBuyDeltaYes, costToBuyDeltaNo, findQuantityForExactCost, priceYes, priceNo } from '../lmsr/lmsr';
...
export default async function marketsRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/markets/:id/buy', async (req, rep) => {
    ...
    let delta = 0;
    let cost = 0;
    let tradeType: 'yes' | 'no' = 'yes';
    
    const MAX_SHARES_PER_TRADE = 10;
    const MAX_COST_PER_MARKET = 100;
    
    if (typeof body.deltaYes === 'number') {
      if (body.deltaYes <= 0) return rep.status(400).send({ error: 'deltaYes must be positive' });
      if (body.deltaYes > MAX_SHARES_PER_TRADE) return rep.status(400).send({ error: `Maximum ${MAX_SHARES_PER_TRADE} shares per trade` });
      tradeType = 'yes';
      delta = body.deltaYes;
      cost = costToBuyDeltaYes(market.qYes, market.qNo, delta, b);
    } else if (typeof body.deltaNo === 'number') {
      if (body.deltaNo <= 0) return rep.status(400).send({ error: 'deltaNo must be positive' });
      if (body.deltaNo > MAX_SHARES_PER_TRADE) return rep.status(400).send({ error: `Maximum ${MAX_SHARES_PER_TRADE} shares per trade` });
      tradeType = 'no';
      delta = body.deltaNo;
      cost = costToBuyDeltaNo(market.qYes, market.qNo, delta, b);
    } else if (typeof body.cost === 'number') {
      if (body.cost <= 0) return rep.status(400).send({ error: 'cost must be positive' });
      if (body.cost > MAX_COST_PER_MARKET) return rep.status(400).send({ error: `Maximum $${MAX_COST_PER_MARKET} per market` });
      tradeType = body.tradeType || 'yes';
      cost = body.cost;
      delta = findQuantityForExactCost(market.qYes, market.qNo, cost, b, tradeType);
      const costFn = tradeType === 'yes' 
        ? (d: number) => costToBuyDeltaYes(market.qYes, market.qNo, d, b)
        : (d: number) => costToBuyDeltaNo(market.qYes, market.qNo, d, b);
      cost = costFn(delta);
    } else {
    ...
    if (user) {
      const spentByUser = db.getTotalSpentByUserOnMarket(user.id, id);
  ...
  fastify.get('/markets/:id/allowance', async (req, rep) => {
    ...
    const market = db.getMarket(id);
    if (!market) return rep.status(404).send({ error: 'market not found' });
    const spent = db.getTotalSpentByUserOnMarket(userId, id);
  ...
  fastify.get('/markets/:id/price', async (req, rep) => {
    ...
    const m = db.getMarket(id);
    if (!m) return rep.status(404).send({ error: 'not found' });
    return { priceYes: priceYes(m.qYes, m.qNo, m.b), priceNo: priceNo(m.qYes, m.qNo, m.b) };
  });
}
